...
int mod(int** maze, int l, int c, int lin, int col,char* modo, int l2, int c2)  { /*escolhe com base na leitura do ficheiro qual da sfunções A chamar*/
    ...
    if((strcmp(modo,"A1"))==0){ /*Lê A1 então chama A1 e as seguintes são semelhantes*/
        ...
        if ((out(l,c,lin,col) ==-2) || (out(l2,c2,lin,col) ==-2)){ /*virifica se as coordenadas indicadas não estão fora do tabuleiro*/
            return -2;
        }
...
void freetabela(int** maze,int lin){    /*liberta todo o espaço alocado para a tabela que constitui o labirinto*/
    for (int i=0; i < lin;i++){
    free(maze[i]);
    }   
    free(maze);
...
int out (int l, int c, int lin, int col) {  /*retorna o número correspondente ao tipo de célula*/
...
void fechar (FILE* fmaze, FILE* fsol){  /*fecha o ficheiro de entrada e de saída*/
    fclose(fmaze);
    fclose(fsol);
}